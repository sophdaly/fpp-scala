...
object Huffman {
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def traverse(codeTree: CodeTree, bits: List[Bit]): List[Char] =
      codeTree match {
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    table.toMap.getOrElse(char, List.empty[Bit])
  ...
  def convert(tree: CodeTree): CodeTable =
    chars(tree).foldLeft(List.empty[(Char, List[Bit])]) {
      (codeTable, char) ⇒ (char, encode(tree)(List(char))) :: codeTable
    }
